...
def insert_function(tablename,rows):
    ...
    #for i in rows:
    #query = "insert into employees.%s values %s" % (tablename,repr(tuple(map(str,[*rows[0]]))))
    #"SELECT * FROM foo WHERE bar IN %s" % repr(tuple(map(str,parameters)))
    #print(query)

    try:
        for i in range(0,len(rows)):
            query = "insert into employees.%s values %s" % (tablename,repr(tuple(map(str,[*rows[i]]))))
...
#q = insert_function('fb_user',rows)
